...
## As yet unreleased
Extract `wheelchair` info for `amenity=fast_food` with `cuisine=kebab` and synonyms.
Extract `wheelchair` info for `amenity=restaurant` with `cuisine=british`, `cuisine=regional` and synonyms.
